...
def _print_fee_rate_drift_summary(prefix: str, fee_rate_drift: dict[str, object]) -> None:
    ...
    print(
        ...
        f"historical_fee_pending_observation_count="
        f"{int(fee_rate_drift.get('historical_fee_pending_observation_count') or 0)} "
        ...
        f"broker_fill_latest_unresolved_fee_pending_count="
        f"{int(fee_rate_drift.get('broker_fill_latest_unresolved_fee_pending_count') or 0)} "
    ...
    print(
        prefix
        + "fee_rate_drift_summary="
        f"{fee_rate_drift.get('diagnostic_only_vs_startup_blocking') or 'unknown'} "
        f"configured_fee_bps={float(fee_rate_drift.get('configured_fee_bps') or 0.0):.3f} "
        f"observed_fee_bps_median={observed_fee_bps_text} "
        f"startup_impact={fee_rate_drift.get('startup_impact') or 'unknown'} "
        f"action={fee_rate_drift.get('operator_action') or 'unknown'}"
    )


def _print_recovery_policy_summary(prefix: str, recovery_policy: dict[str, object]) -> None:
    print(
        prefix
        ...
        f"flatten_not_primary={1 if bool(recovery_policy.get('flatten_not_primary')) else 0}"
    )
    print(
        prefix
    ...
    incident_reasons = ", ".join(str(item) for item in (recovery_policy.get("incident_reasons") or [])) or "none"
    print(prefix + f"incident_reasons={incident_reasons}")
...
def cmd_validate_db(*, as_json: bool = False) -> int:
    ...
    if as_json:
        ...
        print(
            ...
            f"expected_schema_version={diagnostics.get('expected_schema_version')} "
            f"observed_schema_version={diagnostics.get('observed_schema_version')} "
            f"accounting_projection_model={diagnostics.get('accounting_projection_model')} "
            f"expected_accounting_projection_model={diagnostics.get('expected_accounting_projection_model')} "
...
def cmd_recovery_report(*, as_json: bool = False) -> None:
    ...
    print(
        "    "
        f"fee_rate_drift_summary={fee_rate_drift.get('diagnostic_only_vs_startup_blocking') or 'unknown'} "
        f"configured_fee_bps={float(fee_rate_drift.get('configured_fee_bps') or 0.0):.3f} "
        f"observed_fee_bps_median={observed_fee_bps_text} "
        f"startup_impact={fee_rate_drift.get('startup_impact') or 'unknown'} "
        f"action={fee_rate_drift.get('operator_action') or 'unknown'}"
...
def cmd_repair_plan(*, as_json: bool = False) -> None:
    ...
    print(
        ...
        f"flatten_not_primary={1 if bool(plan.get('flatten_not_primary')) else 0} "
        f"non_mutating_preview={1 if bool(plan.get('non_mutating_preview')) else 0}"
    )
    print(
        "  "
    ...
    incident_reasons = ", ".join(str(item) for item in (plan.get("incident_reasons") or [])) or "none"
    print(f"  incident_reasons={incident_reasons}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    research_registry_inspect = sub.add_parser(
        "research-registry-inspect",
        help="inspect one research attempt registry row by hash",
    )
    research_registry_inspect.add_argument("--row-hash", required=True)

    research_registry_validate = sub.add_parser(
        "research-registry-validate",
        help="validate completed registry binding for an experiment id",
    )
    research_registry_validate.add_argument("--experiment-id", required=True)

    research_mark_aborted = sub.add_parser(
        "research-mark-attempt-aborted",
        help="append an aborted event for an incomplete counted research attempt",
    )
    research_mark_aborted.add_argument("--row-hash", required=True)
    research_mark_aborted.add_argument("--reason", required=True)
    ...
    elif args.cmd == "research-registry-inspect":
        return cmd_research_registry_inspect(row_hash=str(args.row_hash))
    elif args.cmd == "research-registry-validate":
        return cmd_research_registry_validate(experiment_id=str(args.experiment_id))
    elif args.cmd == "research-mark-attempt-aborted":
        return cmd_research_mark_attempt_aborted(row_hash=str(args.row_hash), reason=str(args.reason))
