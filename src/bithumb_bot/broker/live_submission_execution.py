    ...
    *,
    ...
    live_module.RUN_LOG.info(
        format_log_kv(
            ...
            execution_engine=str(decision_observability.get("execution_engine") or "-"),
            execution_submit_plan_source=str(decision_observability.get("execution_submit_plan_source") or "-"),
            execution_submit_plan_authority=str(decision_observability.get("execution_submit_plan_authority") or "-"),
            target_delta_side=str(decision_observability.get("target_delta_side") or "-"),
            target_delta_qty=float(decision_observability.get("sell_qty_basis_qty") or 0.0)
            if str(decision_observability.get("execution_engine") or "") == "target_delta"
            else 0.0,
            target_delta_notional_krw=decision_observability.get("target_delta_notional_krw"),
            target_delta_intent_type=str(decision_observability.get("target_delta_intent_type") or "-"),
            target_delta_strategy_context=str(decision_observability.get("target_delta_strategy_context") or "-"),
            target_delta_idempotency_key=str(decision_observability.get("target_delta_idempotency_key") or "-"),
