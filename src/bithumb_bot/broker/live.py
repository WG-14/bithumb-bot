...
def _resolve_submit_qty_source_truth_source(
    *,
    ...
    if normalized_submit_qty_source == "position_state.normalized_exposure.sellable_executable_qty":
...
def _load_strategy_decision_observability(
    *,
    ...
    position_state_source = str(
        ...
        or "position_state.normalized_exposure.sellable_executable_lot_count"
        or "-"
    )
...
def _sell_dust_analysis_qty(*, raw_total_asset_qty: float, open_exposure_qty: float, dust_tracking_qty: float) -> float:
    """Return the raw holdings quantity to use for SELL dust analysis."""
...
def _harmless_dust_suppression_submit_qty_source(submit_qty_source: str | None) -> str:
    ...
    if not normalized_submit_qty_source or normalized_submit_qty_source == "-":
        return "position_state.normalized_exposure.sellable_executable_qty"
...
def _record_sell_dust_unsellable(
    *,
    ...
    resolved_sell_submit_qty_source = "position_state.normalized_exposure.sellable_executable_qty"
    sell_qty_basis_qty = float(
    ...
    suppression_context = {
        ...
        "sell_open_exposure_qty": float(0.0 if open_exposure_qty is None else open_exposure_qty),
...
def _submit_via_standard_path(
    *,
    ...
    sell_observability = _sell_submit_observability_fields(
        decision_observability=decision_observability,
        submit_qty_source=submit_qty_source,
        submit_lot_source=str(decision_observability.get("sell_submit_lot_source") or decision_observability.get("submit_lot_source") or "position_state.normalized_exposure.sellable_executable_lot_count"),
        position_state_source=position_state_source,
    ...
    try:
        ...
        if signal == "BUY" and normalized_exposure.effective_flat:
            ...
            if (not exit_sizing.allowed) or float(order_qty) < sellable_threshold:
                ...
                dust_submit_qty_source = _sell_dust_analysis_source(
                    raw_total_asset_qty=float(raw_total_asset_qty),
                    dust_tracking_qty=float(normalized_exposure.dust_tracking_qty),
                )
                ...
                if str(normalized_exposure.exit_block_reason) == "reserved_for_open_sell_orders":
                    if _record_sell_dust_unsellable(
                        ...
                        ts=int(ts),
                        market_price=float(market_price),
                        position_qty=float(dust_analysis_qty),
                        decision_observability=decision_observability,
                        submit_qty_source=dust_submit_qty_source,
                ...
                if _record_harmless_dust_exit_suppression(
                    ...
                    signal=signal,
                    side=side,
                    requested_qty=float(dust_analysis_qty),
                    market_price=float(market_price),
                    normalized_qty=float(suppression_preview["normalized_qty"]),
                    submit_qty_source=dust_submit_qty_source,
                    position_state_source=dust_submit_qty_source,
                    raw_total_asset_qty=float(raw_total_asset_qty),
                    open_exposure_qty=float(normalized_exposure.open_exposure_qty),
                    dust_tracking_qty=float(normalized_exposure.dust_tracking_qty),
                    strategy_name=strategy_name,
                ...
                if float(raw_total_asset_qty) <= POSITION_EPSILON:
                    skip_reason = (
                        ...
                        f"raw_qty_open={float(open_exposure_qty):.12f} "
            ...
            if not exit_sizing.allowed:
                RUN_LOG.info(
                    format_log_kv(
                        ...
                        final_submitted_qty=float(order_qty),
                    )
                )
                return None
            submit_qty_source = str(exit_sizing.qty_source)
        ...
        if side == "SELL" and not harmless_dust_checked:
        ...
        try:
            ...
            if side == "SELL" and not harmless_dust_checked and _record_harmless_dust_exit_suppression(
                conn=conn,
                state=state,
                signal=signal,
                side=side,
            ...
            RUN_LOG.info(
                format_log_kv(
                    ...
                    signal=signal,
                    side=side,
                    reason=f"category={_classify_sell_failure_category(reason_code=DUST_RESIDUAL_UNSELLABLE, dust_details=getattr(e, 'details', None))};detail={_sell_failure_detail_from_observability(sell_failure_category=_classify_sell_failure_category(reason_code=DUST_RESIDUAL_UNSELLABLE, dust_details=getattr(e, 'details', None)), dust_details=getattr(e, 'details', None))};reason={e}",
                    market_price=market_price,
                    requested_qty=requested_order_qty,
                    position_qty=requested_order_qty,
