...
class _PlaceOrderPlan:
    ...
    submit_price_tick_policy: _SubmitPriceTickPolicy
...
class BithumbBroker:
        ...
        try:
            ...
            if buy_submit_contract is not None:
                chance_validation_order_type = (
                    buy_submit_contract.chance_validation_order_type
                )
                chance_supported_order_types = (
                    buy_submit_contract.chance_supported_order_types
                )
            elif price is None:
    ...
    def _build_place_order_payload(self, *, plan: _PlaceOrderPlan) -> _PlaceOrderPayloadPlan:
        from .order_rules import (
        ...
        if plan.requested_price is None:
            if plan.normalized_side == "bid":
                ...
                exchange_submit_notional = self._decimal_from_value(plan.effective_market_price) * self._decimal_from_value(plan.internal_lot_qty)
                bid_price_unit = self._decimal_from_value(plan.submit_price_tick_policy.price_unit)
                if bid_price_unit > 0:
                ...
                payload = build_order_payload(
                    ...
                    side=plan.normalized_side,
                    ord_type="market",
                    volume=self._format_volume(plan.exchange_submit_qty),
                    client_order_id=plan.validated_client_order_id,
                )
        else:
            requested_limit_price = self._decimal_from_value(plan.requested_price)
            ...
            exchange_submit_notional = normalized_limit_price * self._decimal_from_value(plan.internal_lot_qty)
            min_total = side_min_total_krw(rules=plan.rules, side=plan.order_side)
            if min_total > 0 and exchange_submit_notional < self._decimal_from_value(min_total):
                raise BrokerRejectError(
    ...
    def _execute_place_order(
        ...
        *,
        ...
        data = self._post_private("/v2/orders", payload_plan.payload, retry_safe=False)
