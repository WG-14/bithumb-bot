    ...
    *,
    ...
    try:
        try:
            ...
            print(
                "  tip: initialize or migrate the runtime DB before generating this report; "
                "schema drift blocks realized performance diagnostics."
            )
            return
    finally:
        conn.close()
    ...
    if not stats:
        ...
        print(
            "  tip: no closed lifecycle rows matched the filters; widen the date range, "
            "check strategy/pair filters, or run after trades have closed."
        )
    else:
        print(
            "  "
    ...
    *,
    ...
    payload = {
        ...
        "operational_stability_boundary": {
            "note": (
                "ops-report, health, and recovery outputs are operational safety evidence. "
                "Expectancy metrics are experiment diagnostics and must not override safety gates."
            )
        },
        "experiment_expectancy_metrics": {
            ...
            "fee_drag_ratio_basis": performance_summary.fee_drag_ratio_basis,
            "fee_to_gross_pnl_ratio": performance_summary.fee_to_gross_pnl_ratio,
            "fee_to_gross_pnl_ratio_basis": performance_summary.fee_to_gross_pnl_ratio_basis,
            "traded_notional_total": performance_summary.traded_notional_total,
...
def cmd_ops_report(*, limit: int = 20) -> None:
    ...
    if not strategy_stats:
        ...
        print(
            "  tip: strategy_context is populated from order intent evidence; "
            "run after strategy decisions and order intents have been recorded."
        )
    else:
    ...
    print(
        "  - strategy-report uses trade_lifecycles for realized gross, fee, and net PnL; "
        "verify lifecycle closure before using it for operator review."
    )
    print(
        "  - ops-report strategy_summary links intent/fill evidence for diagnostics; "
        "pnl_proxy_deprecated is not accounting authority."
    )
