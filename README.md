...
- Bithumb `/v2/orders` payload 규칙: 문서 필드(`market`, `side`, `volume`, `price`, `ord_type`) 기준으로 전송합니다. 시장가 매수는 `side=bid`, `ord_type=price`, `price=<총 주문금액 KRW>`로 전송하고 `volume`은 보내지 않습니다. 시장가 매도는 `side=ask`, `ord_type=market`, `volume=<매도 수량>`을 사용합니다.
