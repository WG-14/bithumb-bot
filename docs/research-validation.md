...
Stored L2 rows are evidence, not a full replay engine. Dataset quality, readiness, and research reports distinguish `l2_depth_rows_available`, `l2_depth_snapshot_count`, `l2_depth_row_count`, first/last depth timestamps, sources, and deterministic depth content hashes from execution-capability fields. `depth_available` is retained only as a compatibility field meaning stored L2 rows exist; it does not mean a depth execution model was used. `full_orderbook_depth_available=false`, `top_of_book_is_full_depth=false`, `queue_position_available=false`, `trade_ticks_available=false`, `market_impact_model_available=false`, and `intra_candle_path_available=false` remain the correct claims unless future code implements and tests those capabilities.
