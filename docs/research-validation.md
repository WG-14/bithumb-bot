...
Each transition verifies the parent profile, reopens and rehashes the parent source promotion artifact, rechecks parent evidence artifact hashes when present, records `parent_profile_hash`, and refuses mode skipping. Evidence is treated as a runtime `reports` artifact: the path must exist, resolve outside the repository, and its byte content hash is stored in the child profile as `paper_validation_evidence_path` / `paper_validation_evidence_content_hash` or `live_readiness_evidence_path` / `live_readiness_evidence_content_hash`. Current custody policy rejects repository-local artifacts and accepts absolute repository-external artifacts, including managed `DATA_ROOT/<mode>/reports/...` paths; operators are responsible for custody of external absolute evidence outside managed roots. Those fields are included in the child profile hash. Current evidence validation is artifact validation only; semantic validation of observation windows, decision counts, blocked counts, or readiness metrics is future work unless an operator performs that review manually. `profile-generate` creates paper profiles only; live-compatible profiles must come from `profile-promote`. Live dry-run startup accepts only a verified `live_dry_run` approved profile selected by `APPROVED_STRATEGY_PROFILE_PATH` or its older alias when the canonical selector is unset. Live armed execution accepts only a verified `small_live` approved profile selected the same way.
