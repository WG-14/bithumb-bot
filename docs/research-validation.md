...
`research-forward-diagnostics` is a diagnostic-only feature-mining command. It
loads manifest-backed `DatasetSnapshot` splits and writes forward-return bucket
diagnostics under managed research report and derived paths. The command is not
...
Direct `research-promote-candidate --validation-run` uses the supplied validation run's effective policy fields after loading the run. If those fields require walk-forward, promotion reopens the current `walk_forward_report.json`, recomputes its canonical hash, and verifies the validation run's `walk_forward_report_hash` against it. Missing or stale validation-run binding fails closed with `validation_run_walk_forward_report_hash_missing` or `validation_run_walk_forward_report_hash_mismatch`. New production-bound validation runs must carry `validation_policy_source` and `validation_policy_required_stage_names`; otherwise promotion fails with `validation_run_policy_fields_missing` instead of falling back to weaker manifest flags. Recovery is to rerun `research-validate` from the fixed manifest and dataset so the validation run and walk-forward evidence are regenerated together, not to edit hashes manually.
...
The current SMA runtime exit-rule set is explicit: `stop_loss`, `opposite_cross`, and `max_holding_time`. `stop_loss` is disabled when `STRATEGY_EXIT_STOP_LOSS_RATIO=0` or the rule is absent from `STRATEGY_EXIT_RULES`, and when active it has deterministic priority before signal-based exits. `opposite_cross` includes the configured minimum take-profit floor, round-trip fee floor, and small-loss tolerance band. There are no standalone `take_profit`, `trailing_stop`, or `momentum_timeout` exit rules in the current strategy contract; those names are rejected by the exit-rule factory and must not be implied in promotion evidence.
