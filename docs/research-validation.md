...
When the raw public orderbook payload includes both `bid_size` and `ask_size` for every orderbook unit, the sync path also stores guarded L2 levels in `orderbook_depth_levels` with side, level index, price, size, cumulative size, cumulative notional, source, and observed timestamp. The top-of-book table remains the compatibility surface. If a payload lacks size fields, depth evidence is not fabricated and `depth_available=false` remains the correct report outcome.
