...
- Dust vocabulary in incident response:
...
- `dust_state=harmless_dust` means broker/local dust matches closely enough to be interpreted as harmless dust. Legacy reconcile metadata may still say `matched_harmless_dust_*` for the same class. Resume is allowed only if `resume_allowed_by_policy=1`, and a fresh BUY is allowed only if `dust_new_orders_allowed=1` and `effective_flat_due_to_harmless_dust=1` as well.
...
- Incident triage order:
  ...
  3. Only treat the position as dust-only when `unresolved_count=0`, `recovery_required_count=0`, `dust_state=harmless_dust`, and `resume_allowed_by_policy=1`.
- Strategy interpretation during triage:
  - if `position.in_position=False` because of harmless dust, the bot has already normalized that remainder as flat for fresh BUY entry
  - if `position.in_position=True`, the bot still carries exposure for BUY-gate purposes and should not be treated as flat just because the dust label looks small
