...
Also review `statistical_validation_required`, `statistical_parameter_grid_size`, `statistical_search_budget`, `statistical_attempt_index`, `statistical_holdout_reuse_count`, `selection_universe_hash`, `candidate_metric_values_hash`, `statistical_metric_value_count`, `statistical_missing_metric_count`, `statistical_evidence_hash`, `evidence_grade`, `statistical_method`, `return_panel_hash`, `return_unit`, `return_panel_observation_count`, `family_trial_registry_prior_hash`, `summary_metric_max_bootstrap_p_value`, `white_reality_check_p_value`, `white_reality_check_method`, `statistical_gate_result`, and `statistical_gate_fail_reasons`. These fields are not cosmetic; production-bound promotion refuses missing, mismatched, incomplete, underreported, stale, screening-grade, unavailable-method, or failed statistical evidence. Promotion recomputes `candidate_metric_values_hash` from the current report candidate list and validates the return-panel/statistical evidence chain, so matching copied hash fields alone do not prove the evidence is current. If `statistical_gate_result=FAIL`, `promotion_allowed` must be `0` and `next_action` must be `do_not_promote_review_statistical_selection`.
...
jq '{required: .statistical_validation_required, eligibility: .promotion_eligibility_gate_result, blocking: .promotion_blocking_reasons, universe: .selection_universe_hash, metric_universe: .candidate_metric_values_hash, metric_count: .metric_value_count, missing_metric_count: .missing_metric_count, evidence: .statistical_evidence_hash, grade: .evidence_grade, statistical_method: .statistical_method, return_panel: .return_panel_hash, return_unit: .return_unit, summary_bootstrap_p: .summary_metric_max_bootstrap_p_value, wrc_p: .white_reality_check_p_value, wrc_method: .white_reality_check_method, limitations: .promotion_grade_limitations, gate: .statistical_gate_result, reasons: .statistical_gate_fail_reasons}' "$DATA_ROOT/paper/reports/research/sma_filter_v1_2026_05/backtest_report.json"
