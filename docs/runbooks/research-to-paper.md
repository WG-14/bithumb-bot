...
The preferred and normal production-bound path is the single validation command. It fixes the manifest hash once, derives required stages from `deployment_tier`, runs readiness, backtest, policy-required walk-forward, promotion generation, and reproduce, then writes a hash-bound `ValidationRun` artifact under the managed runtime `reports` bucket unless `--out` is supplied.
...
Review `validation_run_path`, `validation_run_hash`, `validation_run_binding_hash`, `validation_policy_source`, `validation_policy_required_stage_names`, `end_to_end_validation_result`, `selected_candidate_id`, `backtest_report_hash`, `walk_forward_report_hash`, `promotion_artifact_hash`, `reproduce_ok`, `fail_closed_reasons`, and `next_required_action`. Also inspect `.stages[]` in `validation_run.json`: each stage records `name`, `required`, `status`, `input_hashes`, `output_hashes`, `artifact_paths`, `artifact_hashes`, and `reasons`. The stage list is the operator-facing custody record for readiness, dataset quality, backtest, final holdout, stress suite, statistical validation, final selection, walk-forward, promotion eligibility, promotion, and reproduce. Effective policy fields such as `effective_walk_forward_required`, `effective_final_holdout_required`, `effective_stress_suite_required`, `effective_statistical_validation_required`, and `effective_final_selection_required` are the production-bound requirement truth for this validation run.

`research-validate` is the preferred production-bound path because it creates the promotion artifact and final validation-run binding as one custody flow without a cyclic hash dependency. Production-bound or paper-candidate promotion requires verified validation-run binding for new artifacts. Direct `research-backtest` and `research-walk-forward` remain diagnostic/development commands; they are not sufficient production-bound promotion evidence by themselves. A successful process exit from a diagnostic command means the diagnostic command completed; it does not mean promotion-grade validation passed. For production-bound manifests, diagnostic commands return non-zero when the standalone report is not promotion-eligible or is marked diagnostic-only. Pending validation-run promotions are not approved-profile source evidence, and operators must not repair validation evidence by editing hashes manually.
...
Standalone `research-backtest` reports include `validation_run_complete=false`, `diagnostic_only=true`, and `standalone_backtest_not_full_validation=true`. If `walk_forward_required=true`, standalone summaries must show `promotion_allowed=0` with `walk_forward_required_but_not_executed_in_this_run`. This standalone marker must not prevent `research-validate` from executing walk-forward; it only says the standalone backtest command is not the full validation lifecycle. If `promotion_allowed=0`, do not run `research-promote-candidate` from the standalone report. `nearest_failed_candidate_id` is diagnostic only and must not be promoted. Screening statistical evidence, including `SCREENING_SUMMARY_BOOTSTRAP`, is not production-bound promotion evidence; regenerate the validation run with real promotion-grade evidence rather than treating screening p-values as WRC, SPA, DSR, or approved-profile evidence.
...
Both evidence sources are hash-verified and bound into the promotion artifact. `research-promote-candidate` recomputes canonical backtest/walk-forward report hashes before binding them into the promotion artifact. It does not trust embedded report `content_hash` fields, and it fails closed with source-specific missing or mismatch reasons when source evidence has drifted. Current-generation promotion artifacts also carry `lineage_hash`, backtest/walk-forward report hashes, manifest and dataset hashes, command-args hash, repository version, candidate profile hash, calibration hash when present, `validation_run_binding_status`, `validation_run_binding_hash`, `validation_run_hash`, `validation_policy_source`, `validation_policy_required_stage_names`, `effective_walk_forward_required`, `effective_final_holdout_required`, `effective_stress_suite_required`, `effective_statistical_validation_required`, `effective_final_selection_required`, manifest requirement flags, and experiment-family observability fields. Effective deployment-tier policy requirements are the promotion truth for production-bound validation; operators must not treat manifest flags that are weaker than that policy as sufficient production evidence. For `research-validate`, the promotion artifact is bound to a stable pre-promotion validation-run hash so the final validation run can later record `promotion_artifact_hash` and reproduce status without creating a cyclic content hash. Standalone `research-promote-candidate --validation-run` must not create a new promotion artifact that conflicts with a validation run already bound to `promotion_artifact_hash`; use the existing validation-run-bound promotion artifact or rerun `research-validate` from the fixed manifest. Do not manually edit hashes. Approved profile generation refuses production-bound promotion artifacts whose validation-run binding is pending, missing, mismatched, or whose content hash does not match the final validation run's recorded `promotion_artifact_hash`. On success, read the printed `has_execution_calibration_warning`, `execution_calibration_warning_reasons`, `validation_run_binding_status`, `validation_run_binding_hash`, `validation_run_hash`, `validation_run_promotion_artifact_hash`, effective policy fields, and `promotion_warnings` lines before moving to artifact review; optional warn-mode calibration breaches can still promote only as research-only diagnostics. They are operator-visible warnings, not approval signals, and cannot satisfy `profile-generate`, `profile-promote`, `live_dry_run`, or `small_live` transition evidence.
...
Verify the profile hash, candidate parameter values, lineage hash, scenario policy counts, final-holdout presence, final-holdout identity hash, final-holdout content hash, final-holdout reuse-key hash, final-holdout fingerprint compatibility field, dataset fingerprint, manifest hash, content hash, backtest evidence source/hash, statistical evidence source/hash, evidence grade, statistical method, return panel hash, return unit, return panel observation count, selection universe hash, candidate metric values hash, metric counts, summary-metric bootstrap p-value, method limitation fields, calibration hash/market/interval when required, experiment family metadata, family registry path/prior hash/row hash, experiment registry path/prior hash/reservation row hash/completion row hash/bound evidence hash/evidence hash phase, computed attempt index, computed holdout reuse count, declared counter fields, `registry_gate_result`, `registry_gate_fail_reasons`, `research_freedom_hash`, and walk-forward evidence source/hash when required. The current generated p-value method is a screening approximation over summary metrics; it is not promotion-grade White's Reality Check, SPA, Deflated Sharpe, or bar-return bootstrap evidence. Promotion-grade WRC artifacts must be recomputable from the bound return panel and sampling contract. Root/simple smoke backtests remain non-promotable. Optional warn-mode calibration breaches are not hard research-only promotion failures, but `has_execution_calibration_warning=true`, `execution_calibration_warning_reasons`, and `promotion_warnings` must appear in the candidate profile, promotion artifact, and successful promotion CLI output, then be reviewed. Required calibration failures still refuse promotion. Approved-profile generation and profile transitions require `production_calibration_policy_result.status=PASS`. Promotion does not edit `.env`, `BITHUMB_ENV_FILE_PAPER`, `BITHUMB_ENV_FILE_LIVE`, or secrets.

If a production-bound run declares stale `attempt_index` or `holdout_reuse_count`, the backtest fails under the registry lock before appending a counted reservation and appends an uncounted `research_attempt_rejected` event with `counted_attempt=false`. Budget excess is rejected the same way with `experiment_registry_budget_exceeded`, `attempt_budget_exceeded`, or `holdout_reuse_budget_exceeded`. Do not fix this by manually raising counters until a report looks good; inspect the registry and decide whether a new hypothesis or new final-holdout period is required.

If a production-bound run is interrupted after reservation, inspect `DATA_ROOT/<mode>/reports/research/_registry/experiment_registry.jsonl` and the CLI fields `experiment_registry_row_hash`, `experiment_registry_completion_row_hash`, `experiment_registry_bound_evidence_hash`, `experiment_registry_evidence_hash_phase`, `final_holdout_identity_hash`, `final_holdout_content_hash`, `computed_attempt_index`, `computed_holdout_reuse_count`, `registry_gate_result`, and `registry_gate_fail_reasons`. Recovery is append-only:
...
`research-registry-validate` prints `validation_scope`. `registry_only` means registry row and lifecycle validation only; `artifact_binding_valid=unknown`, `evidence_loaded=false`, and `warning=artifact_binding_not_checked` are not promotion-grade artifact validation. `registry_and_artifacts` means the command loaded `backtest_report.json` and checked report/evidence/return-panel binding where available, with explicit `report_loaded`, `evidence_loaded`, `return_panel_loaded`, `artifact_binding_valid`, and `reasons` fields. Aborted events remain counted attempts and are not promotion-permitted. Registry lifecycle status is `IN_PROGRESS`, `COMPLETED`, `ABORTED`, or `REJECTED`; statistical `PASS`/`FAIL` is recorded separately as `statistical_gate_result`. Do not manually edit JSONL rows or hashes.
...
The report must use `comparison_contract_version=canonical_decision_v1`, `canonical_schema=true`, and `promotion_grade_comparison=true`, and must include `research_export_content_hash` and `runtime_export_content_hash`. A `decision_contract_version=1` payload is not promotion evidence unless required semantic fields are present, each decision is bound to the requested profile/market/interval/data fingerprint, runtime order-rule identity is populated from an actual non-empty order-rule snapshot hash, and promotion-grade export decisions include explicit `position_authority`. `position_authority.state_class` must be non-empty, and its `position_state_hash`, `order_rules_hash`, and `fee_authority_hash` must match the enclosing decision. The current repo-owned positive-supported classes are `flat_no_dust_no_position` and `open_exposure`. `reserved_exit_pending` may be classified by the runtime adapter, but it is scaffolded-only and fail-closed until repo-owned runtime-replay fixture coverage passes. Runtime-only states with dust, reserved exit without fixture evidence, non-executable residue, recovery blockers, or lot-native executable authority that research cannot represent must fail closed instead of being treated as equivalent.

Review `claims_scope`, `state_coverage_matrix`, and `outcome` before using the report in a profile transition. Transition evidence must be scope-aware and positive: `outcome=PASS_POSITIVE_EQUIVALENCE`, `ok=true`, `promotion_grade_comparison=true`, non-empty `claims_scope.positive_equivalence_state_classes`, empty `claims_scope.unsupported_state_classes`, and `claims_scope.fail_closed_unmodeled_state_count=0`. `PASS_POSITIVE_EQUIVALENCE` supports only explicitly modeled state classes. `FAIL_CLOSED_UNMODELED_STATE` for dust, residue, non-executable-position, recovery-blocked, or any unsupported state preserves safety but is not transition evidence and is not full lifecycle equivalence evidence. Do not treat partial `lot_native_simulation_v1` support as proof that every paper/runtime order lifecycle, dust handling, pending sell, partial fill, or restart recovery state matches research. Full lifecycle equivalence is not proven unless `claims_scope.full_lifecycle_equivalence_supported=true`.

The current custody policy rejects repository-local artifacts and accepts absolute repository-external artifacts, including managed `DATA_ROOT/<mode>/reports/...` paths; operators remain responsible for custody of external absolute source/evidence artifacts outside managed roots. It stores resolved evidence path, `sha256:` content hash, the decision-equivalence report path/hash, copied decision-equivalence scope fields, and the approved profile hash that the evidence validated, and those fields are included in the child profile hash. Promotion fails closed on malformed evidence, profile/source hash mismatch, insufficient observation window, insufficient decision or closed lifecycle count, execution-quality breaches, unresolved orders, recovery blockers, runtime/profile drift, missing decision-equivalence evidence, decision-equivalence hash mismatch, wrong decision-equivalence profile, market, interval, dataset or comparable DB fingerprint, legacy or unverified decision-equivalence export, missing research/runtime export hashes, incomplete canonical decisions, non-promotion-grade decision-equivalence, blocked decision equivalence, missing scope fields, non-positive outcome, unsupported/fail-closed state scope, non-empty missing research/runtime decision lists, or nonzero decision mismatch count. Rerun decision-equivalence if profile, market, interval, dataset, or DB fingerprints drift.
