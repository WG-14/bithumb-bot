from __future__ import annotations

from types import SimpleNamespace

from bithumb_bot.operator_closeout import build_operator_clean_closeout_contract
from bithumb_bot.quantity_contract import ExchangeQuantityContract
from bithumb_bot.residual_disposition import build_residual_disposition


def test_residual_verdict_includes_quantity_rule_sources():
    verdict = build_residual_disposition(
        residual_inventory=SimpleNamespace(residual_qty=0.00009996, exchange_sellable=False),
        residual_inventory_state="RESIDUAL_INVENTORY_TRACKED",
        residual_policy_allows_run=True,
        residual_policy_allows_buy=True,
        residual_policy_allows_sell=False,
        position_state=SimpleNamespace(
            normalized_exposure=SimpleNamespace(
                has_executable_exposure=False,
                sellable_executable_lot_count=0,
            )
        ),
        authority_assessment={},
        projection_convergence={
            "converged": True,
            "portfolio_qty": 0.00009996,
            "projected_total_qty": 0.00009996,
        },
        broker_position_evidence={"broker_qty_known": True, "broker_qty": 0.00009996},
        lot_definition=SimpleNamespace(
            min_qty=0.0001,
            qty_step=0.0001,
            min_notional_krw=5000.0,
            max_qty_decimals=8,
            source_mode="ledger",
        ),
        open_order_count=0,
        submit_unknown_count=0,
        recovery_required_count=0,
    )

    payload = verdict.as_dict()
    assert payload["min_qty"] == 0.0001
    assert payload["min_qty_source"] == "persisted_exchange_snapshot"
    assert payload["qty_step"] == 0.0001
    assert payload["qty_step_authority_level"] == "persisted_exchange_snapshot"
    assert payload["quantity_contract_complete"] is True


def test_operator_output_includes_quantity_rule_sources():
    verdict = build_residual_disposition(
        residual_inventory=SimpleNamespace(residual_qty=0.00009996, exchange_sellable=False),
        residual_inventory_state="RESIDUAL_INVENTORY_TRACKED",
        residual_policy_allows_run=True,
        residual_policy_allows_buy=True,
        residual_policy_allows_sell=False,
        position_state=SimpleNamespace(
            normalized_exposure=SimpleNamespace(
                has_executable_exposure=False,
                sellable_executable_lot_count=0,
            )
        ),
        authority_assessment={},
        projection_convergence={
            "converged": True,
            "portfolio_qty": 0.00009996,
            "projected_total_qty": 0.00009996,
        },
        broker_position_evidence={"broker_qty_known": True, "broker_qty": 0.00009996},
        lot_definition=SimpleNamespace(
            min_qty=0.0001,
            qty_step=0.0001,
            min_notional_krw=5000.0,
            max_qty_decimals=8,
            source_mode="ledger",
        ),
        open_order_count=0,
        submit_unknown_count=0,
        recovery_required_count=0,
    )

    payload = {
        "quantity_rule_authority": verdict.quantity_rule_authority,
        "qty_step_authority_level": verdict.qty_step_authority_level,
        "quantity_rule_source_mode": verdict.quantity_rule_source_mode,
        "quantity_contract_complete": verdict.quantity_contract_complete,
    }

    assert payload["quantity_rule_authority"] == "persisted_exchange_snapshot"
    assert payload["qty_step_authority_level"] == "persisted_exchange_snapshot"
    assert payload["quantity_rule_source_mode"] == "persisted_exchange_snapshot"
    assert payload["quantity_contract_complete"] is True


def test_local_fallback_qty_min_blocks_as_local_policy_not_exchange_reject():
    contract = ExchangeQuantityContract.local_fallback(
        market="BTC_KRW",
        min_qty=0.0001,
        min_notional_krw=5000.0,
        max_qty_decimals=8,
        configured_qty_step=0.0001,
    )

    payload = contract.as_dict()
    assert payload["qty_step_authority_level"] == "local_fallback"
    assert payload["quantity_rule_source_mode"] == "local_fallback"
    assert contract.quantity_contract_recommended_action == "review_local_quantity_fallback_before_real_submit"


def test_local_fallback_closeout_block_is_not_exchange_reject():
    contract = ExchangeQuantityContract.local_fallback(
        market="BTC_KRW",
        min_qty=0.0001,
        min_notional_krw=5000.0,
        max_qty_decimals=8,
        configured_qty_step=0.0001,
    )
    result = build_operator_clean_closeout_contract(
        broker=object(),
        market="BTC_KRW",
        raw_total_asset_qty=0.00009996,
        broker_asset_available=0.00009996,
        market_price=50_000_000.0,
        quantity_contract=contract,
        dry_run=True,
        plan_place_order_fn=lambda *_args, **_kwargs: None,
        rules=SimpleNamespace(),
        client_order_id="unit",
    ).as_dict()

    assert result["closeout_allowed"] is False
    assert result["block_reason"] == "local_policy_block"
    assert result["recommended_action"] == "review_local_quantity_fallback"
    assert result["quantity_authority"]["qty_step_authority_level"] == "local_fallback"


def test_unknown_qty_authority_blocks_with_refresh_rules_action():
    resolution = SimpleNamespace(
        rules=SimpleNamespace(
            market_id="BTC_KRW",
            min_qty=0.0001,
            min_notional_krw=5000.0,
            qty_step=0.0001,
            max_qty_decimals=8,
        ),
        source={},
        source_mode="",
        snapshot_persisted=False,
    )
    contract = ExchangeQuantityContract.from_rule_resolution(resolution, market="BTC_KRW")

    assert contract.qty_step_authority_level == "unknown"
    assert contract.quantity_contract_complete is False
    assert contract.quantity_contract_recommended_action == "refresh_order_rules_or_review_quantity_settings"


def test_unknown_quantity_authority_blocks_operator_submit():
    resolution = SimpleNamespace(
        rules=SimpleNamespace(
            market_id="BTC_KRW",
            min_qty=0.0001,
            min_notional_krw=5000.0,
            qty_step=0.0001,
            max_qty_decimals=8,
        ),
        source={},
        source_mode="",
        snapshot_persisted=False,
    )
    contract = ExchangeQuantityContract.from_rule_resolution(resolution, market="BTC_KRW")

    result = build_operator_clean_closeout_contract(
        broker=object(),
        market="BTC_KRW",
        raw_total_asset_qty=0.0002,
        broker_asset_available=0.0002,
        market_price=50_000_000.0,
        quantity_contract=contract,
        dry_run=True,
        plan_place_order_fn=lambda *_args, **_kwargs: None,
        rules=SimpleNamespace(),
        client_order_id="unit",
    ).as_dict()

    assert result["closeout_allowed"] is False
    assert result["block_reason"] == "quantity_step_authority_unknown_or_fallback"
    assert result["recommended_action"] == "refresh_order_rules_or_review_quantity_settings"
    assert result["quantity_authority"]["qty_step_authority_level"] == "unknown"
