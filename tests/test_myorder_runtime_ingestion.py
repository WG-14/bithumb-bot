...
def test_myorder_runtime_ingestion_material_zero_fee_live_marks_recovery_required(tmp_path) -> None:
    ...
    assert "fee validation blocked ledger apply" in str(order_row["last_error"])
    assert fill_count == 0
    assert int(stream_row["applied"]) == 1
    assert stream_row["applied_status"] == "recovery_required"
